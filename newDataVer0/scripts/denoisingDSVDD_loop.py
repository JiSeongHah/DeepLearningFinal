...
class denoisingDsvddLoop:
    ...
    def runTrain(self, dataSet):
        ...
        if self.config['data_type'] in [f'800_0_noiseRatio_{round(0.1*i,1)}' for i in range(10)]+[f'800_45_noiseRatio_{round(0.1*i,1)}' for i in range(10)]:
            ...
            if self.config['do_zScore']:
                self.scaler = load(
                    f'./800_Testbed/pkled_data/FEed_data/{whichData_1}/trainVal_{dataName_1}_noised_{actualNoise_1}_scaler.joblib'
                )
    ...
    def load_model(self):
        ...
        if ae_path_exist == False:
            raise Exception('ae path doesnt exist!')
        ...
        main_model_path_exist = os.path.exists(os.path.join(mainModelSavePath, "mainModel_" + str(mainModelIterNum))+ ".pt")
        if main_model_path_exist == False:
            raise Exception("main model saved path doesnt exist! ")
